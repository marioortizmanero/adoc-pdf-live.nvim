...
[source,lua]
----
-- Execute command on BufEnter
vim.api.nvim_create_autocmd('BufEnter', {
  pattern = {"*.adoc", "*.asciidoc"},
  callback = function()
    local cmd = string.format("silent! !pkill -f '%s %s.pdf'", require('adoc_pdf_live').options.viewer, vim.fn.expand('%:r'))
    vim.cmd(cmd)

    vim.fn.luaeval('require("adoc_pdf_live")').open()
  end,
})

# Execute command on quit
